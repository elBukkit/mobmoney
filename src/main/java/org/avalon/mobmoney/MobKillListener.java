...
public class MobKillListener implements Listener {
	...
	public MobKillListener(MobMoney instance) { plugin = instance; }
	...
	public void onMobKill(EntityDeathEvent e) {
		if (e.getEntity().getKiller() instanceof Player) {
			...
			if (spawnedFromSpawner)return;
			if (!plugin.CheckCreative && e.getEntity().getKiller().getGameMode() == GameMode.CREATIVE)return;
			if (plugin.blacklist.contains(e.getEntity().getWorld().getName()))return;
			...
			if (plugin.max.containsKey(e.getEntityType().name())) {
				...
				e.getEntity().getWorld().dropItem(e.getEntity().getLocation(), Item(e.getEntityType().name()));
					} else {
					e.getEntity().getWorld().dropItem(e.getEntity().getLocation(), Item(e.getEntityType().name()));
					e.getEntity().getWorld().dropItem(e.getEntity().getLocation(), Item(e.getEntityType().name()));
					}
